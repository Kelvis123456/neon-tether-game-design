...
### Fixed (Phase 10 — Android rebuild + live playtest, BUG-010)
- Rebuilt and reinstalled the Android debug APK with the difficulty/combo/tutorial changes and the BUG-009 fix, then played it live on a GPU-accelerated emulator: menu, all 5 tutorial steps (danger pose now visibly reaching the obstacle edge, confirmed on-device), and several real gameplay runs. Default (merged) state correctly crashes into a center/magenta obstacle; holding wide (split) now correctly crashes into a side/cyan obstacle — confirming BUG-009's fix works in live play, not just in the isolated test.
- BUG-010: found via logcat during that playtest — haptic feedback (`_haptic()`, called on crash/crystal collection) silently failed on every Android build so far (`SecurityException: VIBRATE permission not found`), since Godot's default export preset doesn't request the VIBRATE permission. Fixed by enabling advanced export options and the Vibrate permission; confirmed granted via `dumpsys package` and the SecurityException gone from logcat after a real in-game crash. See `BUGS.md` and `README.md`.

### Fixed (Phase 10 — BUG-009, side obstacles were unkillable)
- Found while re-verifying the tutorial rewrite below, after the user pointed out that clicking/holding in the test screenshots never actually moved the sphere near an obstacle's edge: side/cyan obstacles could never collide with the player, period, regardless of input. `MAX_WIDTH` (140) capped the tether's reach 45px short of the `SIDE_SAFE_MARGIN` (110) threshold on this 450px canvas — the same bug exists in the original browser prototype (`prototype/app.js`), not something the Godot port introduced. Raised `MAX_WIDTH` to 290 so a full split now actually crosses the threshold on both sides. Verified with a direct in-engine test driving `_move_and_collide()` with contrived states (4/4 cases pass: wide-vs-side now collides, narrow-vs-side safe, narrow-vs-center still collides, wide-vs-center still safe) and the rebuilt tutorial visibly showing the sphere overlap each obstacle's edge in the wrong pose. See `BUGS.md` BUG-009.
