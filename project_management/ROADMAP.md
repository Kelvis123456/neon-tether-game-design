...
- [x] Android debug build — configured the Android SDK/Java paths already present on this machine and produced a real signed `neon-tether-debug.apk` (arm64-v8a + x86_64). Installed and played it on a GPU-accelerated emulator (real Intel UHD 620 via host passthrough): menu, navigation, and a full gameplay run all render and play correctly, with best score persisting across runs. Switched the project's renderer to `gl_compatibility` after `mobile` (Vulkan) kept failing to present specifically on this emulator's Vulkan swapchain — see `BUGS.md` ENV-001 (and BUG-007 for a self-inflicted regression of this same fix along the way).
- [x] VFX pass matching `docs/art_direction.md` section 2 (Tether Ribbon Trails, The Snap Flash, Shatter Spark Burst) and a real app icon replacing the default Godot one — the two biggest gaps between "functional prototype" and "matches the design docs" that didn't require external art/audio tools. See `TASK_LIST.md`.
- [x] Difficulty ramp (speed + spawn rate scaling with distance), a combo multiplier that actually affects score instead of being cosmetic, and a tutorial that teaches the obstacle-color logic (not just raw input) — three design gaps a full live playtest surfaced. See `TASK_LIST.md` and `CHANGELOG.md`.
...
- [ ] iOS build — needs a Mac, unavailable in this dev environment.
