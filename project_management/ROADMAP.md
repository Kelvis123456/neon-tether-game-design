...
- [x] Android debug build — configured the Android SDK/Java paths already present on this machine and produced a real signed `neon-tether-debug.apk` (arm64-v8a + x86_64). Installed and launched it on an emulator; the Godot engine fully initializes with zero crashes (confirmed via logcat), though the emulator's software-only GPU (SwiftShader) couldn't get a final frame to present — see `BUGS.md` ENV-001. Needs a real device or GPU-accelerated emulator to confirm on-screen rendering.
